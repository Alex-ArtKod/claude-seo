...
- Blocks needing an expert voice have an attribution assigned, and the assigned
  expert matches the rules.md attribution map (right person for the topic)?
...
```json
{
  ...
  "checks": {
    ...
    "expert_attribution": "pass | fail — note",
  ...
  "required_revision_instructions": [
    "Block 3: add a comparison table for X vs Y per TZ",
    "Move expert quote in Block 5 from {wrong} to {correct} per rules.md"
  ],
