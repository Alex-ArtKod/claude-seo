...
name: rating-writer
description: >
  Autonomous rating/listicle article writer. Runs in a project folder of TZ
  subfolders and writes each one to a publication-ready ranking article ("ТОП-N",
  comparison of companies) + DOCX, fully unattended. The main company is always
  ranked #1, justified through criteria; competitors are ranked honestly below.
  No quotes anywhere except an optional editorial note by the rating authors about
  the rating itself. The editor (not the user) approves the plan. /rating-writer N
  writes exactly N articles (also max parallelism); default N=1. Distilled inputs
  only (project-context.md + rules.md + participants.md + criteria.md + TZ). Use
  when user says write rating, напиши рейтинг, ТОП компаний, сравнение компаний,
  or runs the pipeline over a project folder of rating TZs.
user-invokable: true
...
# Rating Writer — Autonomous Ranking Pipeline
...
participants, exports to `.docx`, and archives the folder into `Готовые/`. It runs
**unattended** across sessions, surviving usage limits.

This is the rating sibling of `seo-writer`. It reuses the proven multi-phase
pipeline (plan → block writing → assemble → review → DOCX) and adds two
rating-specific stages: **ranking construction** and **competitor safety**.
...
| `{folder}/article-final.md` exists, no `.docx` | Crashed after review; resume at Phase 6–7. |
...
3. **Dispatch up to N** writer agents, one per folder, via the Agent tool with
   ...
   rating-writer/SKILL.md for this folder, end to end. Do not ask questions."**
...
`competitor-safety.md`. **Do not read heavy research files.** Create
`{folder}/.in_progress` if absent.
...
3. **Fact-check** — apply `fact-checker.md` and output its JSON. rules.md /
   ...
   win on conflict. Collect facts worth promoting (for Phase 6).
...
article-final.md + {title}.docx + recom_rules.md + pipeline-log.md). Remove
`.in_progress`. Print:
...
2. Use the `/schedule` skill to create a cron routine that re-invokes
   `/rating-writer {N}` on this project folder **every 2 hours**.
...
| `article-final.md` | 5 | Approved final rating (Markdown) |
